...
fn main() -> ! {
    ...
    loop {
        defmt::debug!("Waiting for for receive interrupt...");

        let mut receiving = match dw1000.receive(RxConfig::default()) {
            Ok(s) => s,
            Err(e) => {
                defmt::panic!("ack");
            }
        ...
        let message = match message {
            Ok(message) => {
                defmt::debug!("Message found");
                message
            }
            Err(_) => {
                defmt::debug!("not a message");
                continue;
            } //ignore error
        };
        ...
        if let Some(response) = response {
            ...
            defmt::debug!("{:04x}:{:04x} - {} mm\n", pan_id.0, addr.0, distance_mm,);

            continue;
        }

        defmt::debug!("Ignored message that was neither ping nor response\n");
    }
}
